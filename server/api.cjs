...
async function initializePluginSystem() {
  try {
    ...
    // Load plugins for ALL companies on startup
    const result = await pool.request().query('SELECT DISTINCT CompanyCode FROM Companies WHERE IsActive = 1');
    const companies = result.recordset.map(row => row.CompanyCode);
    
    console.log(`🏢 Loading plugins for ${companies.length} active companies: ${companies.join(', ')}`);
    
    for (const companyCode of companies) {
      try {
        await pluginManager.initialize(companyCode);
        console.log(`✅ Loaded plugins for ${companyCode}`);
      } catch (err) {
        console.error(`❌ Failed to load plugins for ${companyCode}:`, err.message);
      }
    }
...
async function attachCompanyCode(req, res, next) {
  ...
  try {
    ...
    // Look up user's CompanyCode, Role, and username from Users table
    const result = await pool.request()
      .input('userId', sql.NVarChar, userId)
      .query('SELECT CompanyCode, Role, username, fullName FROM Users WHERE ID = @userId AND IsActive = 1');
    ...
    if (result.recordset.length === 0) {
      if (headerCompanyCode) {
        ...
        req.userName = req.headers['x-user-name'] || null;
        return next();
      }
    ...
    req.userName = user.fullName || user.username || req.headers['x-user-name'] || null;
    
    console.log(`🔐 Data isolation: User ${userId} (${req.userName}) restricted to company: ${user.CompanyCode} (${user.Role})`);
...
app.use((req, res, next) => {
  console.log(`📥 ${req.method} ${req.path} - Headers: userId=${req.headers['x-user-id']}, role=${req.headers['x-user-role']}, name=${req.headers['x-user-name']}`);
  next();
});
...
// Get all tickets (filtered by CompanyCode and role)
app.get('/api/tickets', async (req, res) => {
  try {
    ...
    console.log('📊 GET /api/tickets - companyCode:', companyCode, 'userRole:', userRole, 'userName:', userName);
    ...
    if (userRole === 'Technician' && userName) {
      ...
      console.log(`🔒 Filtering tickets for Technician: ${userName}`);
    } else {
      console.log(`✅ Showing all tickets for role: ${userRole || 'unknown'}`);
    }
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Query user by username - get their company code from the database
    const result = await pool.request()
    ...
    // Use the first active user found (should be unique by username)
    const user = result.recordset[0];
...
app.post('/api/tickets/:ticketId/attachments', upload.single('file'), async (req, res) => {
  try {
    ...
    try {
      ...
      if (tableCheck.recordset[0].tableExists > 0) {
        // Get CompanyCode from request context
  const companyCode = req.userCompanyCode || req.headers['x-company-code'];
