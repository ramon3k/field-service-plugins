...
export default function App() {
  ...
  const [tab, setTab] = useState<'Tickets'|'Map'|'Calendar'|'Reports'|'Closed'|'Users'|'Activity'|'Customers'|'Sites'|'Licenses'|'Vendors'|'Requests'|'Plugins' | string>('Tickets')
  const [currentUser, setCurrentUser] = useState<AuthUser | null>(null)
  const [currentTenant, setCurrentTenant] = useState<TenantInfo | null>(null)
  const [companyDisplayName, setCompanyDisplayName] = useState<string>('Field Service Management') // Dynamic company branding
  ...
  useEffect(() => {
    ...
    const fetchPluginNavTabs = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/plugins/nav-tabs`, { headers })
        if (response.ok) {
          const data = await response.json()
          setPluginNavTabs(data.tabs || [])
        }
      } catch (error) {
