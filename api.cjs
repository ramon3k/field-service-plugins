...
async function initializeDatabase() {
  try {
    ...
    if (isLocalDB && err.message.includes('msnodesqlv8')) {
      console.error('💡 Hint: Install msnodesqlv8 for Windows authentication: npm install msnodesqlv8');
    }
    throw err;
  }
}
...
async function validateUserId(userId) {
  ...
  try {
    ...
    if (result.recordset.length === 0) {
      console.warn(`⚠️ User ID ${userId} not found in Users table, using admin_001 as fallback`);
      return 'admin_001';
    }
...
async function attachCompanyCode(req, res, next) {
  ...
  try {
    ...
    if (!userId) {
      console.warn('⚠️ No user ID in request headers - data isolation may be compromised');
    ...
    if (result.recordset.length === 0) {
      if (headerCompanyCode) {
        console.warn(`⚠️ User ${userId} not found - falling back to provided company header ${headerCompanyCode}`);
        req.userCompanyCode = headerCompanyCode;
        req.userCompanyName = headerCompanyName || null;
    ...
    console.log(`🔒 Data isolation: User ${userId} restricted to company: ${user.CompanyCode} (${user.Role})`);
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    console.log(`👤 User found: ${user.Username} (${user.Role}) in company: ${resolvedCompanyCode}`);
    
    // Verify password (base64 encoded in database)
    const passwordBase64 = Buffer.from(password).toString('base64');
    
    console.log(`🔑 Password check: Provided hash=${passwordBase64}, Stored hash=${user.PasswordHash}, Match=${user.PasswordHash === passwordBase64}`);
    ...
    try {
      ...
      console.log(`🏢 Company branding: ${companyDisplayName}`);
    } catch (companyErr) {
...
app.get('/api/licenses', async (req, res) => {
  try {
    ...
    if (!companyCode) {
      console.warn('⚠️ No CompanyCode found for licenses query');
      return res.status(403).json({ error: 'Company access required' });
    }
...
app.get('/api/licenses/:id', async (req, res) => {
  try {
    ...
    if (!companyCode) {
      console.warn('⚠️ No CompanyCode found for license query');
      return res.status(403).json({ error: 'Company access required' });
    }
...
app.delete('/api/licenses/:id', async (req, res) => {
  try {
    ...
    // Check if license exists and belongs to the company
    const checkResult = await pool.request()
      .input('licenseId', sql.NVarChar, id)
      .input('companyCode', sql.NVarChar, companyCode)
      .query('SELECT LicenseID, CompanyCode FROM Licenses WHERE LicenseID = @licenseId');
    ...
    if (license.CompanyCode !== companyCode) {
      return res.status(403).json({ error: 'Access denied: License belongs to another company' });
    }
...
app.delete('/api/vendors/:id', async (req, res) => {
  try {
    ...
    const checkResult = await pool.request()
      .input('vendorId', sql.NVarChar, id)
      .input('companyCode', sql.NVarChar, companyCode)
      .query('SELECT VendorID, CompanyCode FROM Vendors WHERE VendorID = @vendorId');
    ...
    if (vendor.CompanyCode !== companyCode) {
      return res.status(403).json({ error: 'Access denied: Vendor belongs to another company' });
    }
...
app.post('/api/service-requests/:id/create-ticket', async (req, res) => {
  try {
    ...
    // Get the count of tickets this month for this company
    const countResult = await pool.request()
      .input('companyCode', sql.NVarChar, resolvedCompanyCode)
      .query(`SELECT COUNT(*) as count FROM Tickets WHERE TicketID LIKE 'TKT-' + @companyCode + '-${dateStamp}-%'`);
    ...
    console.log('Creating ticket with ID:', ticketID, 'for company:', resolvedCompanyCode);    // Create ticket with all required fields matching Tickets table schema
...
${sr.IssueDescription}`;
    ...
    // Company code was already resolved and validated above
